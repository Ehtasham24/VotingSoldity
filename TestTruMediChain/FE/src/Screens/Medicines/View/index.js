...
// Screens/Medicines/View.js
import React, { useState, useEffect } from "react";
import { connect } from "react-redux";
import MedicineCard from "../../../Common/MedicineCard";
import Swal from "sweetalert2";
import { MOCK_MEDICINES } from "../MedicineMock";
...
const ViewMedicines = ({ user, history }) => {
  ...
  const deleteMedicine = (medicineId) => {
    ...
    Swal.fire("Deleted!", "Medicine removed successfully", "success");
  };
    ...
    <div className="d-flex flex-row flex-wrap">
      {medicines.length === 0 ? (
        ...
        medicines.map((medicine) => (
          <div key={medicine.medicine_id} className="d-flex m-2">
            <MedicineCard
              {...medicine}
              goToAddStocks={() => history.push("/stocks/add")}
              onDelete={() => deleteMedicine(medicine.medicine_id)}
            />
          </div>
        ))
      )}
