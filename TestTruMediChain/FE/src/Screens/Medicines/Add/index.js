...
const AddMedicine = ({ user, history }) => {
  const [formData, setFormData] = useState({
    medicine_id: generateUniqueId(),
    name: "",
    description: "",
    category: "",
    manufacturer: user ? user.name : "Demo Pharma",
    formula: "",
    antibiotic: "",
    chemicals: "",
  });
  ...
  const createMedicine = async () => {
    const requiredFields = ["name", "description", "category", "formula"];
    ...
    const newMedicine = {
      ...formData,
      chemicals: formData.chemicals.split(",").map((c) => c.trim()),
    };

    console.log("Mock Medicine Created:", newMedicine);
    setLoading(false);

    Swal.fire({
      title: "Success!",
      text: "Medicine created successfully (mock)",
      icon: "success",
      confirmButtonText: "Ok",
    }).then(() => {
      history.push("/medicine");
                ...
                <Col md={6}>
                  <FormGroup check>
                    <Label check>
                      <Input
                        checked={antibiotic === "antibiotic"}
                        onChange={() => antibioticToggle("antibiotic")}
                        type="radio"
                        name="medicineType"
                      />{" "}
                      Antibiotic
                  ...
                  <FormGroup check>
                    <Label check>
                      <Input
                        checked={antibiotic === "non-antibiotic"}
                        onChange={() => antibioticToggle("non-antibiotic")}
                        type="radio"
                        name="medicineType"
                      />{" "}
                      Non-Antibiotic
