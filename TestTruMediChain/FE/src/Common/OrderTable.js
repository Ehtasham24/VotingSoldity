...
const TableItem = ({ order, onClick, updatingOrderIds }) => {
  const { medicine_id, price, distributor, quantity, status, order_id } = order;

  const handleAccept = () => {
    onClick("accepted", order_id);
  };

  const handleReject = () => {
    onClick("rejected", order_id);
  };

  const isUpdating = updatingOrderIds[order_id];
  ...
  return (
    <tr>
      ...
      <td className="text-center">{price} BNB</td>
      <td className="text-center">{quantity}</td>
      <td className="text-center">
        {status.toLowerCase() === "pending" && (
          <div className="badge badge-warning">{status}</div>
      ...
      <td className="text-center">
        {status.toLowerCase() === "pending" ? (
          isUpdating ? (
            <div className="text-muted">Updating...</div>
          ) : (
            <div>
              <button
                onClick={handleAccept}
